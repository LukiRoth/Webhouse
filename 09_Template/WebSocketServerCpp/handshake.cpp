...
int decode_incoming_request (char *coded_request, char **request, int size){
    *request = (char *) malloc(sizeof(char) * size + 2);
    if (*request == NULL) {
        return (-1);
    }
...
int code_outgoing_response (char *response, char **coded_response, int *size){
    *size = strlen (response);

    *coded_response = (char *) malloc(sizeof(char) * (*size + 2));
    if (*coded_response == NULL) {
        return (-1);
    }
